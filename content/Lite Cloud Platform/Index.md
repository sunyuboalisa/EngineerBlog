...
流量接入层：身份识别与路由
七层网关
- **租户上下文提取**：网关解析 JWT 或 Cookie，提取 `tenant_id`。
- **染色与透传**：网关会将租户信息放入 HTTP Header（如 `X-Tenant-Id`）或分布式链路追踪（Trace）上下文中。后续所有的微服务调用都会“染色”，确保下游知道自己在为哪个租户服务。
- **动态路由**：对于 VIP 租户，网关直接将其流量路由到专属的物理集群（Cluster Isolation），而普通租户则进入共享集群。
计算层

存储层
- 租户存储元数据

| **模式**                   | **隔离程度**             | **成本** | **运维复杂度** | **适用场景**          |
| ------------------------ | -------------------- | ------ | --------- | ----------------- |
| **全物理隔离 (Silo)**         | 最高 (独立DB/实例)         | 高      | 高         | 合规要求极高的大型企业、私有化部署 |
| **逻辑隔离 (Bridge/Schema)** | 中 (相同实例，不同Schema)    | 中      | 中         | 中型SaaS，性能要求较高     |
| **数据行隔离 (Pool)**         | 最低 (字段级 `tenant_id`) | 低      | 低         | 初创项目、B2C 变体、海量小客户 |